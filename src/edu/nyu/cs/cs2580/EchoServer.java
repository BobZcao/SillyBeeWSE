...
class EchoHandler implements HttpHandler {
  ...
  private final String searchKey = "/search?query=";
  public void handle(HttpExchange exchange) throws IOException {
    ...
    plainResponse = exchange.getRequestURI().toString();
    if(plainResponse.length() >= searchKey.length()){
      if(plainResponse.substring(0,searchKey.length()).equals(searchKey)){
        plainResponse = plainResponse.substring(searchKey.length(),plainResponse.length());
        plainResponse = plainResponse.replaceAll("\\+"," ");
      }
    }
    plainResponse += '\n';
